...
interval = 0.01
duration = 7200
mosquitto_process_name = "mosquitto-server"
mosquitto_port = 6379
...
def plot_data():
    ...
    # without pert
    zones = [
        (0, 46, 'Loading', 'lightblue'),
        (46, 54, 'Pause', 'lightgray'),
        (54, 69, 'Dump', 'lightgreen'),
        (69, 76, 'Pause', 'lightgray'),
        (76, 106, 'Perturbation', 'lightcoral'),
        (106, 112, 'Pause', 'lightgray'),
        (112, 181, 'Dump (Double Swap)', 'lightgreen')
    ]
    ...
    # print("Nombre d'éléments par zone pour Memory Usage (Mosquitto):")
    ...
    plt.plot(time_in_minutes_mem, memory_usage, label='Memory Usage Mosquitto (MB)', color='blue')
    ...
    plt.title('Memory Usage by Mosquitto and Disturber')
    ...
    plt.title('Disk Activity by Mosquitto')
    plt.legend(loc="upper right")
    plt.xticks(range(int(time_in_minutes[-1]) + 1))
    plt.tight_layout()
    ...
    plt.title('CPU Usage by Mosquitto')
    plt.legend(loc="upper right")
    plt.xticks(range(int(time_in_minutes[-1]) + 1))
    plt.tight_layout()
