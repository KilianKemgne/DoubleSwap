...
uintptr_t align_to_page_start(void *address) {
...
void log_virtual_address(uintptr_t address, const char *tag) {
	FILE *log_file = fopen("/usr/vm/data/guest_data.txt", "a+"); 
	if (log_file == NULL) {
	...
	fprintf(log_file, "%lu %s %02ld:%02ld:%09ld\n",
			address,
			tag,
			(ts.tv_sec / 3600) % 24,
			(ts.tv_sec / 60) % 60,
...
static int persist__client_messages_save(FILE *db_fptr, struct mosquitto *context, struct mosquitto_client_msg *queue)
{
    ...
    // /* Variables pour le suivi du pourcentage et du temps */
    // struct timeval start_time, current_time;
    // gettimeofday(&start_time, NULL); // Temps de départ fixe
    // unsigned long long keys_processed = 0;
    // unsigned long long keys_to_process = db.msg_store_count / 10; // 10% of the total keys
    // int current_percentage = 10; // Commence à 10%

	assert(db_fptr);
	assert(context);
	
	// printf("======= Nombre de messages dans le broker : %d\n", db.msg_store_count);
	cmsg = queue;
	while(cmsg){
		...
		// uintptr_t value_start_page = align_to_page_start((void *)cmsg->store->payload);
        // uintptr_t value_end_page = (align_to_page_start((void *)cmsg->store->payload) + cmsg->store->payloadlen) & ~((uintptr_t)page_size - 1);

		// for (uintptr_t page = value_start_page; page <= value_end_page; page += (uintptr_t)page_size) {
        //     log_virtual_address(page, "OUT G* NONE");
        // }
        ...
        // /* Check if we have processed the current percentage of keys */
        // if (keys_processed >= keys_to_process) {
        //     gettimeofday(&current_time, NULL);
        //     long elapsed_time = (current_time.tv_sec - start_time.tv_sec); // Temps écoulé depuis le début

        //     /* Write the elapsed time and percentage to the file */
        //     FILE *fp = fopen("/usr/vm/dump_percentage.txt", "a");
        //     if (fp != NULL) {
        //         fprintf(fp, "%ld %d\n", elapsed_time, current_percentage);
        //         fclose(fp);
        //     }

        //     /* Reset the keys_processed counter for the next interval */
        //     // keys_processed = 0;

        //     /* Update the percentage and keys_to_process for the next interval */
        //     current_percentage += 10;
        //     keys_to_process = (db.msg_store_count * current_percentage) / 100;
        // }

		cmsg = cmsg->next;
	}
    // printf("======= Nombre de clients obtenues après ittération : %d\n", key_number);
...
static int persist__message_store_save(FILE *db_fptr)
{
    ...
    long keys_to_process = (db.msg_store_count - 53) / 10; // 10% of the total keys
    int current_percentage = 10; // Commence à 10%
	printf("\n======= Nombre de messages dans le broker : %d\n", db.msg_store_count - 53);
	...
	while(stored){
		...
		uintptr_t value_start_page = align_to_page_start((void *)stored->payload);
        uintptr_t value_end_page = (align_to_page_start((void *)stored->payload) + stored->payloadlen) & ~((uintptr_t)page_size - 1);

		uintptr_t value_start_page1 = align_to_page_start((void *)stored);
        uintptr_t value_end_page1 = (align_to_page_start((void *)stored) + sizeof(stored)) & ~((uintptr_t)page_size - 1);
		// printf("\n======= value_start_page : %ld\n", value_start_page);
		...
		for (uintptr_t page = value_start_page1; page <= value_end_page1; page += (uintptr_t)page_size) {
            log_virtual_address(page, "OUT G* NONE");
        }
        ...
        if (keys_processed >= keys_to_process) {
            ...
            /* Update the percentage and keys_to_process for the next interval */
            current_percentage += 10;
            keys_to_process = ((db.msg_store_count - 53) * current_percentage) / 100;
        }
		
		stored = stored->next;
	}
    printf("======= Nombre de messages obtenues après ittération : %d\n", key_number);
