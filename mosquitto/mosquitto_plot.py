...
def plot_data():
    ...
    # plt.xticks(range(int(time_in_minutes[-1]) + 1))
    # plt.tight_layout()
    # plt.savefig('mosquitto_cpu_usage_with_children.png')
    # plt.show()
    ...
    # plt.xticks(range(int(time_in_minutes[-1]) + 1))
    # plt.tight_layout()
    # plt.savefig('mosquitto_network_io.png')
    # plt.show()
...
if __name__ == "__main__":
    try:
        ...
        child_thread = threading.Thread(target=collect_child_data, args=(mosquitto_proc, interval, duration))
        child_thread.start()

        print(f"Collecte des données pendant {duration} secondes...")
        collect_data(mosquitto_proc, interval, duration)
