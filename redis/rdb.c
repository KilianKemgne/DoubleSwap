 ...
 */
 ...
 uintptr_t align_to_page_start(void *address) {
     ...
     // }
     return (uintptr_t)address & ~(4096 - 1);
 }
 
 void log_virtual_address(uintptr_t address, const char *tag) {
     FILE *log_file = fopen("/usr/vm/guest_data.txt", "a+"); 
     if (log_file == NULL) {
 ...
 // void log_in_virtual_address(uintptr_t address, const char *tag) {
 //     FILE *log_file = fopen("/usr/vm/data/guest_in_data.txt", "a+"); 
 //     if (log_file == NULL) {
 //         perror("Failed to open log file");
 //         return;
 //     }
...
ssize_t rdbSaveDb(rio *rdb, int dbid, int rdbflags, long *key_counter) {
    ...
    printf("======= Nombre de clés dans la base de données : %llu \n", db_size);
    ...
    unsigned long long keys_to_process = db_size / 10; // 10% of the total keys
    ...
    while ((de = kvstoreIteratorNext(kvs_it)) != NULL) {
        ...
        uintptr_t keystr_start_page = align_to_page_start((void *)keystr);
        uintptr_t keystr_end_page = (align_to_page_start((void *)keystr) + sdslen(keystr)) & ~(page_size - 1);
        // if(object_length == 0) { 
        //     printf("======= key : %ld   value : %ld\n", sdslen(keystr), sdslen(o->ptr));
        // }
        // object_length+=sdslen(keystr);

        uintptr_t value_start_page = align_to_page_start((void *)o->ptr);
        uintptr_t value_end_page = (align_to_page_start((void *)o->ptr) + sdslen(o->ptr)) & ~(page_size - 1);

        // object_length+=sdslen(o->ptr);        

        for (uintptr_t page = keystr_start_page; page <= keystr_end_page; page += page_size) {
            log_virtual_address(page, "OUT G* NONE");
        }

        for (uintptr_t page = value_start_page; page <= value_end_page; page += page_size) {
            log_virtual_address(page, "OUT G* NONE");
        }
        ...
        if (keys_processed >= keys_to_process) {
            ...
            keys_to_process = (db_size * current_percentage) / 100;
        }
    }
    printf("======= Nombre de clés obtenues après ittération : %d\n", key_number);
    // printf("======= Nombre de clés obtenues après ittération en utilisant la taille des données : %ld\n", object_length/4096);
...
werr:
 ...
 int rdbLoadRioWithLoadingCtx(rio *rdb, int rdbflags, rdbSaveInfo *rsi, rdbLoadingCtx *rdb_loading_ctx) {
     ...
     while(1) {
         ...
         // uintptr_t keystr_start_page = align_to_page_start((void *)key);
         // uintptr_t keystr_end_page = (align_to_page_start((void *)key) + sdslen(key)) & ~(page_size - 1);
 
         // for (uintptr_t page = keystr_start_page; page <= keystr_end_page; page += page_size) {
         //     log_in_virtual_address(page, "IN G NONE");
         // }
     }
