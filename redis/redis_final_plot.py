...
def collect_data(proc, interval, duration):
    ...
    while time.time() - start_time < duration:
        ...
        # network_io = get_network_io_for_port(redis_port)
        # network_io_sent.append(network_io['sent'] / (1024 * 1024))
        # network_io_recv.append(network_io['recv'] / (1024 * 1024))
...
# def get_network_io_for_port(port):
#     sent = 0
#     recv = 0
#     for conn in psutil.net_connections(kind='inet'):
#         if conn.laddr.port == port:
#             if conn.status == psutil.CONN_ESTABLISHED:
#                 if hasattr(conn, 'bytes_sent') and hasattr(conn, 'bytes_recv'):
#                     sent += conn.bytes_sent
#                     recv += conn.bytes_recv
...
def save_data_to_files():
    ...
    # with open('network_io_sent.txt', 'w') as network_sent_file:
    #     for timestamp, sent in zip(timestamps, network_io_sent):
    #         network_sent_file.write(f"{timestamp.strftime('%H:%M')}: {sent}\n")

    # with open('network_io_recv.txt', 'w') as network_recv_file:
    #     for timestamp, recv in zip(timestamps, network_io_recv):
    #         network_recv_file.write(f"{timestamp.strftime('%H:%M')}: {recv}\n")
