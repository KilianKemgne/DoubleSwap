...
def plot_data():
    ...
    zones = [
        ...
        (0, 48, 'Loading (Memcached)', 'lightblue'),
        (48, 58, 'Pause', 'lightgray'),
        (58, 80, 'Dump Memcached (With CRIU)', 'lightgreen'),
        (80, 87, 'Pause', 'lightgray'),
        (87, 129, 'V(dis)', 'lightcoral'),
        (129, 137, 'Pause', 'lightgray'),
        (137, 257, 'Dump Memcached (With CRIU)', 'lightgreen')
       # (345, 1103, 'Dump (Redis after perturbation)', 'lightgreen'),
    ]
    ...
    plt.plot(time_in_minutes_mem, memory_usage, label='Memory Usage Memcached (MB)', color='blue')
    ...
    plt.title('Memory Usage by Memcached and V(dis)')
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(time_in_minutes_io, disk_io_read, label='Disk Read Memcached (MB)', color='blue')
    # for pid, data in children_data.items():
    #     time_in_minutes_child = [(t - start_time).total_seconds() / 60 for t in data['timestamps']]
    ...
    plt.title('Disk Activity by Memcached')
    plt.legend(loc="upper right")
    plt.xticks(range(int(time_in_minutes[-1]) + 1))
    plt.tight_layout()
    ...
    plt.plot(time_in_minutes_truncated, cpu_usage_cpu2, label='CPU Usage Memcached (%)', color='blue')
    for pid, data in children_data.items():
    ...
    plt.title('CPU Usage by Memcached')
    plt.legend(loc="upper right")
    plt.xticks(range(int(time_in_minutes[-1]) + 1))
    plt.tight_layout()
