...
/***
 * 
 * Code for writting into a file the address mapping between GPA and HVA
 * 
*/

static void write_gpa_hva(struct kvm_userspace_memory_region mem, bool new, ram_addr_t addr) {
    pthread_mutex_lock(&mutex);

    int fd = open("/tmp/hgfile.txt", O_WRONLY | O_CREAT | O_APPEND, S_IRUSR | S_IWUSR);
    if (fd == -1) {
        perror("Erreur lors de l'ouverture du fichier");
        exit(EXIT_FAILURE);
    }

    // char *data = mmap(NULL, 256, PROT_WRITE, MAP_SHARED, fd, 0);
    // if (data == MAP_FAILED) {
    //     perror("Erreur lors du mappage du fichier");
    //     exit(EXIT_FAILURE);
    // }

    printf("%llu %llu %llu %llu %s\n",mem.userspace_addr,mem.guest_phys_addr,mem.memory_size, addr, new ? "true" : "false");

    // int ret = snprintf(data, 256, "%llu %llu", mem.userspace_addr, mem.guest_phys_addr);
    // if (ret < 0 || ret >= 256) {
    //     fprintf(stderr, "Erreur lors de l'écriture des données\n");
    //     exit(EXIT_FAILURE);
    // }

    char buf[256];
    int ret = snprintf(buf, sizeof(buf), "%llu %llu %llu %llu\n", mem.userspace_addr, mem.guest_phys_addr,mem.memory_size, addr);
    if (ret < 0 || ret >= sizeof(buf)) {
        fprintf(stderr, "Erreur lors de la génération des données\n");
        exit(EXIT_FAILURE);
    }

    ssize_t bytes_written = write(fd, buf, ret);
    if (bytes_written < 0) {
        perror("Erreur lors de l'écriture dans le fichier");
        exit(EXIT_FAILURE);
    }

    // if (munmap(data, 256) == -1) {
    //     perror("Erreur lors de la libération du mappage mémoire");
    //     exit(EXIT_FAILURE);
    // }
...
static int kvm_set_user_memory_region(KVMMemoryListener *kml, KVMSlot *slot, bool new)
{
    ...
    /******** Modification ********/
    write_gpa_hva(mem, new, slot->ram_start_offset);
    /******** End of Modification ********/
err:
...
int kvm_init_vcpu(CPUState *cpu, Error **errp)
{
    ...
    cpu->kvm_run = mmap(NULL, mmap_size, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE | MAP_PRIVATE,
                        cpu->kvm_fd, 0);
    ...
    if (s->kvm_dirty_ring_size) {
        ...
        cpu->kvm_dirty_gfns = mmap(NULL, s->kvm_dirty_ring_bytes,
                                   PROT_READ | PROT_WRITE, MAP_SHARED | MAP_POPULATE | MAP_PRIVATE,
                                   cpu->kvm_fd,
