...
static void log_swap_out_work_func(struct work_struct *work) {
    ...
    snprintf(buf, sizeof(buf), "%lu %s %02lu:%02lu:%09lu\n",
             data->page_addr, data->up ? "OUT G* NONE" : "OUT G NONE", hours, minutes, nanoseconds);

    file = filp_open("/usr/data/guest_data.txt", O_CREAT | O_WRONLY | O_APPEND, 0644);
    if (IS_ERR(file)) {
        pr_err("Could not open /usr/data/guest_data.txt for logging: error %ld\n", PTR_ERR(file));
        kfree(data);
        return;
    }
    ...
    if (written < 0) {
        pr_err("Failed to write to /usr/data/guest_data.txt: error %ld\n", written);
    }
...
static void log_swap_out_info(unsigned long page_addr, bool up) {
...
{
	log_swap_out_info(page_to_pfn(&folio->page), true);
	/*
	...
	if (folio_clear_dirty_for_io(folio)) {
		...
		log_swap_out_info(page_to_pfn(&folio->page), false);
		return PAGE_SUCCESS;
	}
