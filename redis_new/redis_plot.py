...
def collect_data(redis_proc, interval, duration):
    """Collecte les données pour Redis, ses enfants et l'application `app`."""
    ...
    while time.time() - start_time < duration:
        ...
        # Collecter les données pour Redis
        if is_process_running(redis_proc):
            cpu_usage.append(redis_proc.cpu_percent(interval=interval))
            memory_usage.append(redis_proc.memory_info().rss / (1024 * 1024))  # en Mo
            disk_io = redis_proc.io_counters()
...
def collect_child_data(redis_proc, interval, duration):
    """Collecte les données pour les processus enfants de Redis."""
    start_time = time.time()
    while time.time() - start_time < duration:
        if not is_process_running(redis_proc):
            print("Le processus Redis n'est plus en cours d'exécution. Arrêt de la collecte des enfants...")
            break

        children = redis_proc.children()
...
def plot_data():
    """Trace les graphiques pour Redis, ses enfants et l'application `app`."""
    ...
    plt.plot(time_in_minutes[:len(memory_usage)], memory_usage, label='Memory Usage Redis (MB)', color='blue')
    ...
    plt.title('Memory Usage by Redis, App, and Redis Children')
...
if __name__ == "__main__":
    try:
        redis_proc = find_process(redis_process_name)
        if not redis_proc:
