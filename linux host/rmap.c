...
/******* Modification *******/
static int get_process_name_using_vma_1(struct vm_area_struct *vma, char *name, char *time, size_t size, size_t time_size) {
...
static void log_swap_out_work_func(struct work_struct *work) {
    ...
    snprintf(buf, sizeof(buf), "%lu %s %02lu:%02lu:%09lu %s %s\n",
             data->page_addr, "OUT H NONE", hours, minutes, nanoseconds,
             data->process_name, data->time);

    file = filp_open("/usr/data/data.txt", O_CREAT | O_WRONLY | O_APPEND, 0644);
    if (IS_ERR(file)) {
        pr_err("Could not open /usr/data/data.txt for logging: error %ld\n", PTR_ERR(file));
        kfree(data);
        return;
    }
    ...
    if (written < 0) {
        pr_err("Failed to write to /usr/data/data.txt: error %ld\n", written);
    }
...
static void log_swap_out_info(unsigned long page_addr, char *process_name, char *time) {
    struct log_work_data *data;

    data = kmalloc(sizeof(*data), GFP_ATOMIC);
    if (!data) {
        pr_err("Failed to allocate memory for log_swap_out_info\n");
        return;
    }
    ...
    INIT_WORK(&data->work, log_swap_out_work_func);
...
//     snprintf(buf, sizeof(buf), "%lu %s %02lu:%02lu:%09lu %s %s\n", page_addr, "OUT H NONE", hours, minutes, nanoseconds, process_name, time);

//     // mutex_lock(&log_mutex);
//     file = filp_open("/usr/data/data.txt", O_CREAT | O_WRONLY | O_APPEND, 0644);
//     if (IS_ERR(file)) {
// 		pr_err("Could not open /usr/data/data.txt: error %ld\n", PTR_ERR(file));
	...
	anon_vma_interval_tree_foreach(avc, &anon_vma->rb_root,
		...
		if (rwc->done && rwc->done(folio)) {
			for (int i = 0; i < folio_nr_pages(folio); i++) {
				if (get_process_name_using_vma_1(vma, process_name, time, sizeof(process_name), sizeof(time)) == 0) {
					/********* Modification ********/
					log_swap_out_info(address + 4096 * i, process_name, time);
					/********* Modification ********/
				} else {
