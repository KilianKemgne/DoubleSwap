...
/******** Modification ********/
static int get_process_name_using_vma(struct vm_area_struct *vma, char *name, char *time, size_t size, size_t time_size) {
...
static void log_swap_in_work_func(struct work_struct *work) {
    struct log_work_data *data = container_of(work, struct log_work_data, work);
    struct file *file;
    char buf[256];
    loff_t pos = 0;
    ...
    unsigned long hours = (ts.tv_sec / 3600) % 24;
    unsigned long minutes = (ts.tv_sec / 60) % 60;
    unsigned long nanoseconds = ts.tv_nsec;

    snprintf(buf, sizeof(buf), "%lu %s %02lu:%02lu:%09lu %s %s\n",
             data->page_addr, data->is_cache ? "IN H CACHE" : "IN H NONE", hours, minutes, nanoseconds,
             data->process_name, data->time);

    file = filp_open("/usr/data/data_in.txt", O_CREAT | O_WRONLY | O_APPEND, 0644);
    if (IS_ERR(file)) {
        pr_err("Could not open /usr/data/data_in.txt for logging: error %ld\n", PTR_ERR(file));
        kfree(data);
        return;
    }
    ...
    if (written < 0) {
        pr_err("Failed to write to /usr/data/data_in.txt: error %ld\n", written);
    }
...
static void log_swap_in_info(unsigned long page_addr, char *process_name, char *time, bool is_cache) {
    struct log_work_data *data;

    data = kmalloc(sizeof(*data), GFP_ATOMIC);
    if (!data) {
        pr_err("Failed to allocate memory for log_swap_in_info\n");
        return;
    }

    data->page_addr = page_addr;
	data->is_cache = is_cache;
    strncpy(data->process_name, process_name, sizeof(data->process_name) - 1);
    strncpy(data->time, time, sizeof(data->time) - 1);

    INIT_WORK(&data->work, log_swap_in_work_func);
    schedule_work(&data->work);
}

static void log_swap_in_wp_work_func(struct work_struct *work) {
    struct log_work_data *data = container_of(work, struct log_work_data, work);
    struct file *file;
    char buf[256];
    loff_t pos = 0;
    ...
    snprintf(buf, sizeof(buf), "%lu %s %02lu:%02lu:%09lu %s %s\n",
             data->page_addr, "IN H WP", hours, minutes, nanoseconds,
             data->process_name, data->time);

    file = filp_open("/usr/data/host_data.txt", O_CREAT | O_WRONLY | O_APPEND, 0644);
    if (IS_ERR(file)) {
        pr_err("Could not open /usr/data/host_data.txt for logging: error %ld\n", PTR_ERR(file));
        kfree(data);
        return;
    }
    ...
    ssize_t written = kernel_write(file, buf, strlen(buf), &pos);
    if (written < 0) {
        pr_err("Failed to write to /usr/data/host_data.txt: error %ld\n", written);
    }
...
static void log_swap_in_info_wp(unsigned long page_addr, char *process_name, char *time) {
    struct log_work_data *data;

    data = kmalloc(sizeof(*data), GFP_ATOMIC);
    if (!data) {
        pr_err("Failed to allocate memory for log_swap_in_info_wp\n");
        return;
    }
    ...
    INIT_WORK(&data->work, log_swap_in_wp_work_func);
    schedule_work(&data->work);
}


// static void log_swap_in_info_wp(unsigned long page_addr, char *process_name, char *time, bool is_cache) {
//     struct file *file;
//     char buf[256];
//     struct timespec64 ts;
// 	loff_t pos = 0;
...
//         pr_err("Invalid arguments to log_swap_in_info_wp\n");
//         return;
//     }
...
//     snprintf(buf, sizeof(buf), "%lu %s %02lu:%02lu:%09lu %s %s\n",page_addr, "IN H WP", hours, minutes, nanoseconds, process_name, time);

//     // mutex_lock(&log_mutex);
//     file = filp_open("/usr/data/host_data.txt", O_CREAT | O_WRONLY | O_APPEND, 0644);
//     if (IS_ERR(file)) {
// 		pr_err("Could not open /usr/data/host_data.txt for swap in log: error %ld\n", PTR_ERR(file));
...
vm_fault_t do_swap_page(struct vm_fault *vmf)
{
	...
	if (folio) {
		...
		for (int i = 0; i < folio_nr_pages(folio); i++) {
			if (get_process_name_using_vma(vma, process_name, time, sizeof(process_name), sizeof(time)) == 0) {
				/********* Modification ********/
				log_swap_in_info(vmf->address + 4096 * i, process_name, time, true);
				/********* Modification ********/
			} else {
				printk(KERN_ERR "No process found for the given VMA\n");
			}
		}
	}
	...
	if (!folio) {
		...
		for (int i = 0; i < folio_nr_pages(folio); i++) {
			if (get_process_name_using_vma(vma, process_name, time, sizeof(process_name), sizeof(time)) == 0) {
				/********* Modification ********/
				log_swap_in_info(vmf->address + 4096 * i, process_name, time, false);
				/********* Modification ********/
			} else {
				printk(KERN_ERR "No process found for the given VMA\n");
			}
...
static vm_fault_t handle_pte_fault(struct vm_fault *vmf)
{
	...
	if (vmf->flags & (FAULT_FLAG_WRITE|FAULT_FLAG_UNSHARE)) {
		if (!pte_write(entry)) {
			if (get_process_name_using_vma(vmf->vma, process_name, time, sizeof(process_name), sizeof(time)) == 0) {
				if(strcmp(process_name, "qemu-system-x86") == 0)
					log_swap_in_info_wp(vmf->address, process_name, time);
			} else {
				pr_err("No process found for the given VMA\n");
			}
